...
use crate::{db::UserExt, dtos::{FilterUserDto, NameUpdateDto, RequestQueryDto, Response, RoleUpdateDto, UserData, UserListResponseDto, UserPasswordUpdateDto, UserResponseDto}, error::{ErrorMessage, HttpError}, middleware::JWTAuthMiddleware, models::UserRole, utils::password, AppState};


pub fn users_handler(app_state: Arc<AppState>) -> Router<Arc<AppState>> {
    Router::new()
        .route("/me", get(get_me))
...
pub async fn update_user_role(
    ...
    let user_role = UserRole::try_from(body.role)
        .map_err(|e| HttpError::bad_request(e.to_string()))?;

    let result = app_state.db_client
        .update_user_role(user_id.clone(), user_role)
        .await
        .map_err(|e| HttpError::server_error(e.to_string()))?;
