...
pub enum ErrorMessage {
    ...
    InvalidHashFormat, // Added this variant
}
...
impl fmt::Display for ErrorMessage {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ErrorMessage::UserNoLongerExists => write!(f, "User belonging to this token no longer exists"),
            ...
            ErrorMessage::ExceededMaxPasswordLength(max_length) => write!(f, "Password must not be more than {} characters", max_length),
            ...
            ErrorMessage::UserNotAuthenticated => write!(f, "Authentication required. Please log in."),
        }
    }
}
...
impl HttpError {
    pub fn new(message: ErrorMessage, status_code: StatusCode) -> Self {
        HttpError { message, status_code }
    }

    pub fn server_error(message: ErrorMessage) -> Self {
        HttpError::from(message)
    }

    pub fn bad_request(message: ErrorMessage) -> Self {
        HttpError::from(message)
    }
...
impl fmt::Display for HttpError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "HttpError: message: {}, status: {}", self.status_code, self.message)
    }
}
